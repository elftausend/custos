...
use crate::{Matrix, libs::opencl::GenericOCL};
...
impl Node {
    pub fn new(out_dims: (usize, usize)) -> Node {
        let node = Node {
            idx: unsafe {CPUCACHE_COUNT},
            out_dims,
        };
        unsafe {CPUCACHE_COUNT+=1};
        node
    }
}

pub static mut CACHE: CPUCache = CPUCache { nodes: None };
