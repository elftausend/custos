...
impl BorrowingCache {
    ...
    pub(crate) fn add_or_get<'a, T, D, S>(&mut self, device: &'a D, id: Ident) -> &Buffer<'a, T, D, S>
    where
        T: 'static,
        D: Alloc<'a, T, S>,
        S: Shape 
    ...
    pub(crate) fn add_or_get_mut<'a, T, D, S>(&mut self, device: &D, id: Ident) -> &mut Buffer<'a, T, D, S>
    where
        T: 'static,
        D: for<'b> Alloc<'b, T, S>,
        S: Shape 
    {
        ...
        unsafe {
            transmute(buf_any.downcast_mut::<Buffer<T, D, S>>().unwrap())
        }
    }
    ...
    {
        unsafe {
            transmute(self.cache.get_mut(&id)?.downcast_mut::<Buffer<T, D, S>>())
        }
    }
}
