...
impl<'dev> core::fmt::Debug for GradientsLT<'dev> {
...
impl<'dev> GradientsLT<'dev> {
    ...
    #[inline]
    pub(crate) unsafe fn may_get_ref<T, S, D>(
        &self,
        ident: Id,
    ...
    #[inline]
    pub(crate) unsafe fn may_get_mut<T, S, D>(
        &mut self,
        id: Id,
    ...
    #[inline]
    pub fn get_ref<T, S, D>(&mut self, device: &'dev D, id: Id) -> &Buffer<'dev, T, D, S>
    where
        T: Unit + 'static,
        S: Shape,
    ...
    /// Allocates a gradient [`Buffer`] if it does not exist.
    #[inline]
    pub fn get_mut<T, S, D>(&mut self, device: &'dev D, id: Id) -> &mut Buffer<'dev, T, D, S>
    where
        T: Unit + 'static,
        S: Shape,
    ...
    #[inline]
    pub fn get_like<T, S, D>(&mut self, buf: &Buffer<'dev, T, D, S>) -> &Buffer<'dev, T, D, S>
    where
        T: Unit + 'static,
        S: Shape,
        D: Alloc<T> + ZeroGrad<T> + 'static,
